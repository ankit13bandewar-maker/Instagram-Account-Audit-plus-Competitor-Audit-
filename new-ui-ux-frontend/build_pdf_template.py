...
<body>
  ...
  <script>
    ...
    function getProxyImgUrl(displayUrl, postUrl, type, index) {
      if (displayUrl && typeof displayUrl === 'string' && displayUrl.startsWith('data:image/')) {
        return displayUrl;
      }
      const isReel = (type && type.toString().toLowerCase().includes('video')) || (type && type.toString().toLowerCase().includes('reel'));
      const label = (index || (isReel ? 'REEL' : 'POST')).toString().toUpperCase();
      const c1 = isReel ? '%238b5cf6' : '%230284c7';
      const c2 = isReel ? '%23d946ef' : '%2338bdf8';
      const pathD = isReel 
        ? "M52 40 L74 52 L52 64 Z" 
        : "M44 42 H76 V62 H44 Z M52 48 A3 3 0 1 0 52 54 A3 3 0 1 0 52 48 M70 60 L62 50 L54 60";

      return `data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='120' height='120' viewBox='0 0 120 120'><defs><linearGradient id='g' x1='0%25' y1='0%25' x2='100%25' y2='100%25'><stop offset='0%25' stop-color='${c1}'/><stop offset='100%25' stop-color='${c2}'/></linearGradient></defs><rect width='120' height='120' rx='12' fill='url(%23g)'/><circle cx='60' cy='52' r='22' fill='white' fill-opacity='0.25'/><path d='${pathD}' fill='white'/><text x='60' y='96' fill='white' font-family='system-ui, -apple-system, sans-serif' font-size='13' font-weight='800' text-anchor='middle' letter-spacing='0.5'>${label}</text></svg>`;
    }
    ...
    async function init() {
      ...
      try {
        ...
        try {
          const urlParams = new URLSearchParams(window.location.search);
          const handle = (urlParams.get('handle') || 'DemoAccount').replace('@','').trim();
          buildPDF(handle, getFallbackAuditData(handle));
          if (overlay) overlay.style.display = 'none';
          const wrapper = document.getElementById('pdf-wrapper');
    ...
    function buildPDF(handle, rawData) {
      ...
      let compMetrics = rawData.competitor_metrics || data.competitor_metrics || [];
      if (!Array.isArray(compMetrics) || compMetrics.length === 0) {
        compMetrics = [
          { competitor_name: "@espncricinfo", rank: 1, follower_count: 8500000, metrics: { average_likes: 42000, average_comments: 650, engagement_rate: 4.2 }, best_post: { url: "https://www.instagram.com/espncricinfo/", likes: 95000, comments: 1200 } },
          { competitor_name: "@bcci", rank: 2, follower_count: 7200000, metrics: { average_likes: 38000, average_comments: 540, engagement_rate: 3.8 }, best_post: { url: "https://www.instagram.com/bcci/", likes: 82000, comments: 980 } },
          { competitor_name: "@icc", rank: 3, follower_count: 6800000, metrics: { average_likes: 31000, average_comments: 480, engagement_rate: 3.5 }, best_post: { url: "https://www.instagram.com/icc/", likes: 71000, comments: 890 } }
        ];
      }
      ...
      p3.innerHTML = `
        ...
        <div class="glass-card" style="margin-bottom: 20px;">
          ...
          <div style="display: flex; flex-direction: colu            ${reelsPosts.slice(0, 4).map(p => `
              <a href="${resolvePostUrl(p)}" target="_blank" rel="noopener noreferrer" style="text-decoration: none; display: flex; justify-content: space-between; align-items: center; padding: 8px 12px; background: #f8fafc; border-radius: 6px; border: 1px solid var(--theme-card-border);">
                <div style="display: flex; align-items: center; gap: 10px;">
                  <img crossorigin="anonymous" src="${getProxyImgUrl(p.display_url || p.displayUrl, p.post_url, 'Video', p.index)}" style="width: 36px; height: 36px; border-radius: 6px; object-fit: cover;" />
                  <div>
                    <div style="font-weight: 700; font-size: 9.5pt; color: var(--theme-ink);">${p.index || 'Reel'} <span style="font-size: 8pt; color: #6b21a8; font-weight: 600;">↗</span></div>
        ...
        <div class="glass-card">
          ...
          <div style="display: flex; flex-direction: column; gap: 8px;">
            ${staticPosts.slice(0, 4).map(p => `
              <a href="${resolvePostUrl(p)}" target="_blank" rel="noopener noreferrer" style="text-decoration: none; display: flex; justify-content: space-between; align-items: center; padding: 8px 12px; background: #f8fafc; border-radius: 6px; border: 1px solid var(--theme-card-border);">
                <div style="display: flex; align-items: center; gap: 10px;">
                  <img crossorigin="anonymous" src="${getProxyImgUrl(p.display_url || p.displayUrl, p.post_url, 'Image', p.index)}" style="width: 36px; height: 36px; border-radius: 6px; object-fit: cover;" />
                  <div>
                    <div style="font-weight: 700; font-size: 9.5pt; color: var(--theme-ink);">${p.index || 'Static Post'} <span style="font-size: 8pt; color: #0284c7; font-weight: 600;">↗</span></div>
                    <div style="font-size: 8pt; color: var(--theme-muted);">${formatDate(p.date || p.timestamp)}</div>
                  </div>
                </div>
            ...
            `).join('') || '<div style="font-size: 9pt; color: var(--theme-muted);">No Static Posts published.</div>'}ublished.</div>'}
          </div>
        </div>
      `;
